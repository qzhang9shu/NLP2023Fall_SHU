...
from prepare_data import PrepareData, DataLoader
...
def main():
    ...
    args.src_vocab = len(data.src_word_dict)
    args.tgt_vocab = len(data.tgt_word_dict)

    print("Size:\nsrc_vocab %d" % args.src_vocab)
    print("tgt_vocab %d\n" % args.tgt_vocab)
    ...
    if args.type == 'train':
        ...
        if os.path.exists(args.save_model):
          print("<<<<<< The pretrained model had already exsisted at %s.\n" % args.save_model)

          if args.pre2train is not False:
            print(">>>>>> Load the pretrained model and continue to train...\n")
            ...
            print("<<<<<< Finished pre2train")

          else:
            print(">>>>>> Loading the pre-trained model at %s...\n" % args.save_model)
            model.load_state_dict(torch.load(args.save_model))
        ...
        if os.path.exists(args.save_model):
            ...
            evaluate(data, model, beam_size = args.beam_size)         
        ...
        if os.path.exists(args.save_model):
            ...
            print("Error: please train before generate")
